...
- `li:first-child`: 1, 4
- `li:last-child`: 5, 6
- `p:first-of-type`: 1, 3, 4
- `p:last-of-type`: 5, 6, 12
- `p:nth-child(2)`: 3, 5
- `p:nth-last-child(2)`: 4
- `p:nth-of-type(2)`: 5, 6, 7
- `p:nth-last-of-type(2)`: 3, 4, 8
- `span:only-child`: 16
- `a:only-of-type`: 2, 17
- `p:eq(3)`: 5
...
|       Selector       |       example        | Mean                                                                                                       |
| :------------------: | :------------------: | :--------------------------------------------------------------------------------------------------------- |
|     [attribute]      |     \$("[href]")     | 所有 href 屬性的元素                                                                                       |
|  [attribute=value]   | \$("[src='a.jpg']")  | 所有 src 屬性`等於` a.jpg                                                                                  |
|  [attribute!=value]  | \$("[src!='a.jpg']") | 所有 src 屬性`不等於` a.jpg (不是 `[src!='a.jpg']` 的都是)                                                 |
|  [attribute$=value]  | \$("[src$='.jpg']")  | 所有 src 屬性`尾端`是 `.jpg`                                                                               |
| [`attribute|=value`] | \$(`[value|='en']`)  | 所有 value 屬性是 `en` (例如: `value="en"`) 或`開頭`是 `en-`(例如: `value="en- example"`)                  |
| [`attribute^=value`] | \$(`[value^='en']`)  | 所有 value 屬性`開頭`是 `en` (開頭包含 en 的都是， `value="en.jpg a a-jpg"` 是，`value="b en-jpg"` 就不是) |
| [`attribute~=value`] | \$(`[value~='en']`)  | 所有 value 屬性包含 `en` 這一個`字` (需要是一個完整的字 `value="a.jpg en a-jpg"` 中有 `en` 就是)           |
| [`attribute*=value`] | \$(`[value~='en']`)  | 所有 value 屬性包含 `en` 這一個`字串` (`value="a.jpg en a-jpg"` 與 `value="en-jpg"` 情形都算)              |
